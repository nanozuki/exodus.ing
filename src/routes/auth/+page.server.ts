...
export const actions = {
  register: async ({ locals, request }) => {
    ...
    const valid = await locals.auth.validateInviteCode(inviteCode);
    if (!valid) {
      return fail(400, { error: { inviteCode: '邀请码无效' } });
    }

    const url = await locals.auth.authByGithub(inviteCode);
    redirect(302, url.toString());
  },
  login: async ({ locals }) => {
    const url = await locals.auth.authByGithub();
    redirect(302, url.toString());
  },
} satisfies Actions;
