...
func migrateData(sqliteDB, pgDB *gorm.DB) error {
	...
	if err := migrateUserDomain(sqliteDB, pgDB); err != nil {
		return err
	}
...
func migrateUserDomain(sqliteDB, pgDB *gorm.DB) error {
	var liteUserDomains []lite.UserDomain
	var pgUserDomains []pg.UserDomain
	if err := sqliteDB.Find(&liteUserDomains).Error; err != nil {
		return err
	}
	for _, liteUserDomain := range liteUserDomains {
		pgUserDomain := pg.UserDomain{
			UserID:          liteUserDomain.UserID,
			Domain:          liteUserDomain.Domain,
			VerifyTxtRecord: liteUserDomain.VerifyTxtRecord,
			VerifiedAt:      time.UnixMilli(liteUserDomain.VerifiedAt).UTC(),
		}
		pgUserDomains = append(pgUserDomains, pgUserDomain)
	}
	if err := pgDB.Create(&pgUserDomains).Error; err != nil {
		return err
	}
	return nil
}
